...
class Adminapi extends Controller
{
    ...
    public function addrecharge(Request $r){
        if($r['type'] == 1){
            addwallet($r['user_id'], $r['amount'],'+');
        }else{
            addwallet($r['user_id'], $r['amount'],'-');
        }
        $response = array('status' => 1, 'title' => "Success!!", 'message' => "Successfully!");
    ...
    public function editbankdetail(Request $r)
    {
        ...
        if (request()->hasFile('barcode')) {
            ...
            $barcode = 'assets/upload/' . $fileName;
        }
    ...
    public function withdrawal_query(Request $r)
    {
        ...
        $time =  date('H:i:s');
        if ("07:00:00" > $time ||  "23:30:00"  < $time) {
            return redirect('/withdraw?msg=error2');
        }

        $firstrecharge11 = Transaction::where('userid', user('id'))->where('category', 'withdraw')->where('status', 0)->count();
        // dd($firstrecharge);
        if ($firstrecharge11 != 0) {
            return redirect('/withdraw?msg=error3');
        }

        $firstrecharge = Transaction::where('userid', user('id'))->where('category', 'recharge')->where('amount', '>=', 100)->where('status', 1)->count();
        // dd($firstrecharge);
        if ($firstrecharge == 0) {
            return redirect('/withdraw?msg=error1');
        }

        $setting1 = Setting::where('id', '5')->first();
        if ($r->amount < $setting1->value) {
    ...
    public function joinuser(Request $request)
    {
        try {
            ...
            $userid = user('id');
            $amount = 10;
            $firstrecharge = Transaction::where('userid', $userid)->where('category', 'join')->where('status', 1)->count();

            if($firstrecharge == 0){
                addtransaction($userid, 'join', date("ydmhsi"), 'credit', $amount, 'join', 'Success', 1);
                addwallet($userid, $amount);
